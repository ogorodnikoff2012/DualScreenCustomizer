  ...
  public void updateVirtualScreenInfo(VirtualScreenState state) {
    ...
    for (var entry : state.virtualScreens.entrySet()) {
      ...
      ScreenDescriptor descriptor =
          ScreenDescriptor.builder().withIdentifier(identifier).withVendorName("VIRTUAL").build();

      Rectangle geometry = asGeometry(screenInfo);

      ScreenInfoRecord record =
          new ScreenInfoRecord(descriptor, geometry, VirtualScreenInfoProvider.DEFAULT_PRIORITY);
