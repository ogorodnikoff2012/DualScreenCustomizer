...
public class EDID {
  ...
  public static EDID parseEDID(byte[] edid) throws EDIDParserException {
    if (edid.length < EDID_LENGTH) {
      throw new EDIDParserException(
          String.format("Bad EDID length, expected 128, got %d", edid.length));
    }

    byte checksum = countChecksum(edid);
    if (checksum != 0) {
      throw new EDIDParserException(
          String.format("Bad EDID checksum, expected 0, got %d", checksum));
    }
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (descriptorType == (byte) 0xFF) {
        displaySerial =
            extractDescriptorText(edid, descriptorBegin + 5, descriptorBegin + descriptorLength);
