...
public class ExternalCall {
	...
	public static CompletableFuture<List<String>> runCmd(String cmd, Consumer<OutputStream> inputCallback) throws IOException {
		Process process = Runtime.getRuntime().exec(cmd);
		if (inputCallback != null) {
			inputCallback.accept(process.getOutputStream());
		}
		return process.onExit().thenApply((p) ->
			new BufferedReader(new InputStreamReader(p.getInputStream())).lines().collect(Collectors.toList())
		);
	}
