...
public class MainListener implements EditorTrackerListener {
  ...
  private static final WeakHashMap<JComponent, ComponentListener> LISTENERS = new WeakHashMap<>();

  private static final ComponentListener DUMMY_COMPONENT_LISTENER =
  ...
  public MainListener(Project project) {
    ...
    projectWindow =
        SwingUtilities.getWindowAncestor(
            Objects.requireNonNull(WindowManager.getInstance().getIdeFrame(project))
                .getComponent());
        ...
        .addUpdateListener(
            new ActionListener() {
              ...
              public void actionPerformed(ActionEvent e) {
                ...
                try {
                  updateSettings(
                      EditorTracker.getInstance(MainListener.this.project).getActiveEditors());
                } catch (Exception ex) {
                  panicFlag = true;
  ...
  private void updateSettings(ScreenInfoRegistry registry, Editor editor) {
    ...
    Point editorCenter =
        new Point((int) editorGeometry.getCenterX(), (int) editorGeometry.getCenterY());
